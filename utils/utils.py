...
def pipeline_wrapper(dic_agent_conf, dic_traffic_env_conf, dic_path, roadnet, trafficflow):
    ...
    for i in range(1):
        ...
        if not isinstance(round_results, dict):
            print(
                "RL training completed, but no evaluation metrics were produced "
                "by Pipeline.run()."
            )
            return
        ...
        # delete junk
        cmd_delete_model = r'find <dir> -type f ! -name "round_<round>_inter_*.h5" -exec rm -rf {} \;'.replace("<dir>", dic_path["PATH_TO_MODEL"]).replace("<round>", str(int(dic_traffic_env_conf["NUM_ROUNDS"] - 1)))
        cmd_delete_work = r'find <dir> -type f ! -name "state_action.json" -exec rm -rf {} \;'.replace("<dir>", dic_path["PATH_TO_WORK_DIRECTORY"])
...
def oneline_wrapper(dic_agent_conf, dic_traffic_env_conf, dic_path, roadnet, trafficflow):
    ...
    table_logger = wandb.init(
        project=dic_traffic_env_conf['PROJECT_NAME'],
        group=f"{dic_traffic_env_conf['MODEL_NAME']}-{roadnet}-{trafficflow}-"
              f"{len(dic_traffic_env_conf.get('PHASE', config.dic_traffic_env_conf['PHASE']))}_Phases",
        name="exp_results",
        config=merge(merge(dic_agent_conf, dic_path), dic_traffic_env_conf),
    )
    columns = ["reward", "avg_queue_len", "avg_travel_time"]
