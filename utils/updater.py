...
class Updater:
    ...
    def __init__(self, cnt_round, dic_agent_conf, dic_traffic_env_conf, dic_path):
        ...
        self.sample_indexes1, self.sample_indexes2 = None, None
        ...
        for i in range(dic_traffic_env_conf['NUM_AGENTS']):
            ...
            agent = DIC_AGENTS[agent_name](
                ...
                self.dic_path, self.cnt_round, str(i))
    ...
    def load_sample_with_forget(self, i):
        ...
        try:
            ...
            sample_size1 = min(self.dic_agent_conf["SAMPLE_SIZE1"], len(memory_after_forget))
            self.sample_indexes1 = random.sample(range(len(memory_after_forget)), sample_size1)
            sample_set = [memory_after_forget[k] for k in self.sample_indexes1]
            print("==== memory samples number =====:",  sample_size1)
            if self.dic_traffic_env_conf['MODEL_NAME']=='DynamicLight':
                if self.cnt_round == self.dic_agent_conf["SROUND"]-1 or self.cnt_round == self.dic_agent_conf["SROUND2"]-1:
                    with open(os.path.join(self.dic_path["PATH_TO_WORK_DIRECTORY"], "train_round",
                                        "total_samples_inter_{0}".format(i) + ".pkl"), "wb+") as f:
                        pickle.dump([], f, -1)
        except:
    ...
    def load_sample_for_agents(self):
        ...
        if self.dic_traffic_env_conf['MODEL_NAME']=='DynamicLight':
            sample_set_all1,  sample_set_all2 = [], []
            for i in range(self.dic_traffic_env_conf['NUM_INTERSECTIONS']):
                sample_set1, sample_set2 = self.load_sample_with_forget(i)
                sample_set_all1.extend(sample_set1)
                sample_set_all2.extend(sample_set2)
            self.agents[0].prepare_Xs_Y(sample_set_all1)
