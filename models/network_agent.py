...
class NetworkAgent(Agent):
    ...
    def load_network(self, file_name, file_path=None):
        ...
        self.q_network = load_model(
            ...
            custom_objects={"Selector": Selector, "slice_tensor": slice_tensor, "relation": relation},
        )
    ...
    def build_network_from_copy(self, network_copy):
        ...
        # MPLight contains Lambda layers that Keras 3 cannot safely reconstruct
        # from JSON without losing output-shape information. The source is an
        # already-built in-process model, so a deep copy is both safer and more
        # faithful than serializing arbitrary Lambda bytecode.
        try:
            network = copy.deepcopy(network_copy)
        except (TypeError, ValueError, AttributeError):
            # Some Keras Lambda graphs cannot be pickled even though they can
            # be constructed normally. Rebuild the graph through the agent's
            # own factory and copy only numeric weights.
