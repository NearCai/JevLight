...
url = os.getenv("CHATGPT_API_BASE_URL", "https://api.deepseek.com/v1")
headers = {
...
def configure_chat_endpoint(api_base=None, api_key=None, timeout=None, fallback=None):
    """Configure the shared OpenAI-compatible client before agents are created."""
    global url, headers
    if api_base:
        url = api_base
    ...
    if timeout is not None:
        os.environ["CHATGPT_TIMEOUT"] = str(timeout)
    if fallback is not None:
        os.environ["CHATGPT_FALLBACK"] = "1" if fallback else "0"


def _fallback_signal(state, phases):
    """Select the phase with the largest urgent stopped-vehicle backlog."""
    phase_names = list(phases)
    scores = []
    for phase in phase_names:
        score = 0.0
        for lane_name in phase_lanes[phase]:
            lane = state.get(lane_name, {})
            queue = float(lane.get("queue_len", 0))
            wait = float(lane.get("avg_wait_time", 0))
            cells = lane.get("cells", ())
            near = sum(float(value) * weight for value, weight in zip(cells, (0.1, 0.3, 0.7, 1.0)))
            score += 10.0 * queue + 0.05 * queue * wait + 2.0 * near
        scores.append(score)
    return phase_names[max(range(len(phase_names)), key=lambda index: (scores[index], -index))]


def _chat_completion(messages, model, timeout):
    """Call an OpenAI-compatible endpoint (SiliconFlow, Ollama, or hosted APIs)."""
    token = os.getenv("CHATGPT_API_KEY", "").strip()
    local_endpoint = "localhost" in url or "127.0.0.1" in url
    if not token and not local_endpoint:
        raise RuntimeError("CHATGPT_API_KEY is not configured")
...
four_phase_list = {'Phase-1': 0, 'Phase-2': 1, 'Phase-3': 2, 'Phase-4': 3}
eight_phase_list = {'Phase-1': 0, 'Phase-2': 1, 'Phase-3': 2, 'Phase-4': 3, 'WTWL': 4, 'ETEL': 5, 'STSL': 6, 'NTNL': 7}
...
phase_explanation_dict_detail = {"Phase-2": "- Phase-2: Northern and southern through lanes.",
                                 "Phase-4": "- Phase-4: Northern and southern left-turn lanes.",
                                 "NTNL": "- NTNL: Northern through and left-turn lanes.",
                                 "STSL": "- STSL: Southern through and left-turn lanes.",
                                 "Phase-1": "- Phase-1: Eastern and western through lanes.",
                                 "Phase-3": "- Phase-3: Eastern and western left-turn lanes.",
...
class ChatGPTTLCS_Wait_Time_Forecast(object):
    def __init__(self, GPT_version, intersection, inter_name, phase_num, log_dir, dataset):
        ...
        self.log_model = re.sub(r"[^A-Za-z0-9_.-]+", "_", str(GPT_version))
        os.makedirs(log_dir, exist_ok=True)
        self.last_action = "Phase-1"
        self.system_prompt = load_json("./prompts/prompt_domain_knowledge.json")["system_prompt"]
        self.state_action_prompt_file = f"{log_dir}/{dataset}-{self.inter_name}-{self.log_model}-{phase_num}_state_action_prompt_domain_knowledge.json"
        self.error_file = f"{log_dir}/{dataset}-{self.inter_name}-{self.log_model}-{phase_num}_error_prompts_domain_knowledge.json"
        self.state_action_prompt = []
        self.errors = []

        self.temp_action_logger = ""
        self.use_fallback = os.getenv("CHATGPT_FALLBACK", "1").lower() not in {"0", "false", "no"}
        self.timeout = float(os.getenv("CHATGPT_TIMEOUT", "30"))
    ...
    def choose_action(self, env):
        ...
        if flow_num == 0:
            action_code = self.action2code("Phase-1")
            self.state_action_prompt.append({"state": state, "prompt": [], "action": "Phase-1"})
        ...
        analysis = None
        try:
            analysis = _chat_completion(prompt, self.gpt_version, self.timeout)
            matches = re.findall(r'<signal>\s*([^<]+?)\s*</signal>', analysis, flags=re.IGNORECASE)
            if matches:
                signal_text = matches[-1].strip()
        except Exception as error:
            self.errors.append({"error": str(error), "prompt": prompt})
            dump_json(self.errors, self.error_file)
            if not self.use_fallback:
                raise
        if signal_text not in self.phases:
            if not self.use_fallback and analysis is not None:
                raise ValueError(f"model returned invalid signal: {signal_text!r}")
            signal_text = _fallback_signal(state, self.phases)
            analysis = f"<fallback>selected the largest urgent backlog</fallback><signal>{signal_text}</signal>"
...
class ChatGPTTLCS_Commonsense(object):
    def __init__(self, GPT_version, intersection, inter_name, phase_num, log_dir, dataset):
        ...
        self.state_action_prompt_file = f"{log_dir}/{dataset}-{self.inter_name}-{self.log_model}-{phase_num}_state_action_prompt_commonsense_no_calculation.json"
        self.error_file = f"{log_dir}/{dataset}-{self.inter_name}-{self.log_model}-{phase_num}_error_prompts_commonsense_no_calculation.json"
        self.state_action_prompt = []
        self.errors = []
