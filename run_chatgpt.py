...
def parse_args():
    ...
    parser.add_argument("--gpt_version", type=str, default=os.getenv("CHATGPT_MODEL", "Qwen/Qwen2.5-7B-Instruct"))
    parser.add_argument("--api_base", type=str, default=os.getenv("CHATGPT_API_BASE_URL", "https://api.siliconflow.cn/v1"),
                        help="OpenAI-compatible base URL; use http://localhost:11434/v1 for Ollama")
    parser.add_argument("--api_key", type=str, default=os.getenv("CHATGPT_API_KEY", ""))
    parser.add_argument("--timeout", type=float, default=float(os.getenv("CHATGPT_TIMEOUT", "30")))
    parser.add_argument("--run_counts", type=int, default=3600)
    parser.add_argument("--no_fallback", action="store_true",
                        help="fail when the configured endpoint is unavailable")
...
def main(in_args):
    ...
    from models.chatgpt import configure_chat_endpoint
    configure_chat_endpoint(
        api_base=in_args.api_base,
        api_key=in_args.api_key,
        timeout=in_args.timeout,
        fallback=not in_args.no_fallback,
    )
